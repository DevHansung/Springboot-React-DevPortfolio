  ...
  onEnabled = (userid) => {
    try {
      uploadUserEnabled(userid)
        .then(res => {
          if (res.enabled === 0) {
            message.success('계정이 활성화 되었습니다.');
          }
          else if (res.enabled === 1) {
            message.success('계정이 비활성화 되었습니다.');
          }
          ...
          message.error("작업을 수행하던 중 문제가 발생하였습니다..")
        });
    } catch (error) {
      message.error(error.message || '다시 시도해주세요..')
    }
  }

  onRoleEdit = (userid) => {
    try {
      uploadRoleEdit(userid, this.props.currentUsername)
        .then(res => {
          if (res.roles[0].name === 'ROLE_ADMIN') {
            message.success("관리자 권한을 부여 했습니다.")
          }
          else if (res.roles[0].name === 'ROLE_USER') {
            message.success("일반 유저 권한을 부여 했습니다.")
          }
          const { userList } = this.state;
          const index = userList.findIndex(user => user.id === userid);
          const nextUser = [...userList];
      ...
      },
      {
        ...
        render: (text, record) => (
          this.props.role === 'ROLE_TOPADMIN' ?
            this.props.currentUsername === record.username ? null :
    ...
    return (
      <div>
        <div className="editList-container">
          <h2 style={{ textAlign: 'center' }}> <strong>회원 관리</strong> </h2>
          <Table rowKey={record => record.username} dataSource={this.state.userList} columns={columns} pagination={{ pageSize: 10 }} />
        </div>
      </div>
    );
  }
