...
class Modal extends Component { 
	...
	render () {
		...
		if (this.state.wasClickedLeft === false && this.state.wasClickedRight === false) {
			modalContent = (
				<div className='Modal'>
					<SignPanel onLogin={this.props.onLogin} type='signIn' onChange={this.onClickLeft}></SignPanel>
					<SignPanel type='signUp' onChange={this.onClickRight}></SignPanel>
				</div>
			);
		} else if (this.state.wasClickedLeft === false && this.state.wasClickedRight === true) {
			modalContent = (
				<div className='Modal'>
					<SignCollapsed onLogin={this.props.onLogin} type='signIn' onChange={this.onClickLeft}></SignCollapsed>
					<SignExpanded type='signUp' signState={this.onClickLeft}></SignExpanded>
				</div>
			);
		} else if (this.state.wasClickedLeft === true && this.state.wasClickedRight === false) {
			modalContent = (
				<div className='Modal'>
					<SignExpanded onLogin={this.props.onLogin} type='signIn' ></SignExpanded>
