...
import { loadPortfolioById, editPortfolio, editPortfolioImage } from '../../controller/APIController';
...
class PortfolioEdit extends Component {
    constructor(props) {
        ...
        this.onEditPortfolio = this.onEditPortfolio.bind(this);
    ...
    loadPortfolioById() {
        loadPortfolioById(this.props.portfolioId)
            .then((res) => {
                this.setState({
                    title: {
                        value: res.title
    ...
    onEditPortfolio() {
        try {
            if (this.state.fileList.length === 0) {
                editPortfolio(this.props.portfolioId, this.state.title.value, this.state.summary.value,
                    this.state.period.value, this.state.technology.value, this.state.github.value)
                    .then((res) => {
                        this.props.onHandleOk(this.props.portfolioId);
                        message.success('요청하신 작업이 완료 되었습니다.');
                    })
            }
            else if (this.state.fileList.length !== 0) {
                editPortfolioImage(this.props.portfolioId, this.state.title.value, this.state.summary.value,
                ...
                <Form>
                    ...
                    <Form.Item label="이미지">
                        <Dragger fileList={this.state.fileList} onChange={this.onChange} beforeUpload={() => false} multiple={true}>
                            ...
                            <p className="ant-upload-hint">Image fils only</p>
                        </Dragger>
                    </Form.Item>
                    <Form.Item>
                        <Button onClick={this.onEditPortfolio} disabled={this.isFormInvalid()} type="primary" className="uploadAddButton" size="large" htmlType="submit">Save</Button>
                    </Form.Item>
                </Form>
            </div>
        );
