...
import { uploadInformation } from '../../controller/APIController';
...
class InformationUpload extends Component {
    constructor(props) {
        ...
        this.onUploadInformation = this.onUploadInformation.bind(this);
    }
    ...
    onUploadInformation() {
        try {
            uploadInformation(this.props.username, this.state.title.value, this.state.summary.value,
                this.state.text.value, this.state.name.value, this.state.email.value, this.state.github.value)
                .then(() => {
                    this.setState({
                        title: {
                            value: ''
                    ...
                    this.props.onSubmit("information");
                }).catch((error) => {
        ...
        return (
            <div className="upload-container">
                <Form>
                    ...
                    <Form.Item>
                        <Button onClick={this.onUploadInformation} disabled={this.isFormInvalid()} type="primary" className="uploadAddButton" size="large" htmlType="submit">Save</Button>
                    </Form.Item>
                </Form>
            </div>
        );
